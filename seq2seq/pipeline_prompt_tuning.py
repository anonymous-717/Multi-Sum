...
from prompt_model import PromptSummarizationModule
import math
...
class ModelArguments:
    ...
    #For prompt model
    promptModel_name_or_path: Optional[str] = field(
        default=None,
        metadata={"help":"Path to pretrained prompt model or model identifier from huggingface.co/models."},
    )    
    preseqlen: int = field(default=200, metadata={"help": "the length of the prefix."})    
    tuning_mode: str = field(default='prompttune', metadata={"help": "Could be prompttune or finetune."})       
...
def main():
    ...
    if model_args.tuning_mode == 'prompttune':
    ...
    #lang_id = 0
    if model_args.promptModel_name_or_path is not None and model_args.load_whole_model:
        print("load whole model from {}".format(model_args.promptModel_name_or_path))
        model = PromptSummarizationModule.from_pretrained(model_args.promptModel_name_or_path,
                            from_tf=bool(".ckpt" in model_args.promptModel_name_or_path),
                            cache_dir=model_args.cache_dir,
                            hparams=model_args, 
                            config=config, 
                            tokenizer=tokenizer, 
        ...
        if model_args.tuning_mode == 'prompttune':
            model = PromptSummarizationModule(config=config, hparams=model_args,  tokenizer=tokenizer, seq2seq_model=seq2seq_model, lang_id=lang_id)
        else:
            model = seq2seq_model
    ...
    if model_args.tuning_mode == 'prompttune' and not model_args.not_freeze_lmodel:
